...
contract Pyrrho is Context, IERC20 {
    ...
    uint256 private constant startingSupply = 5_000_000_000_000;
    ...
    Fees public _taxRates = Fees({buyFee: 200, sellFee: 400});

    Ratios public _ratios = Ratios({cake: 33, bnb: 33, busd: 33, total: 99});

    StaticValues public staticVals =
        StaticValues({
            maxBuyTaxes: 1500,
            maxSellTaxes: 1500,
    ...
    function isExcludedFromWalletSize(address account)
        public
        view
        returns (bool)
    {
    ...
    function setExcludedFromWalletSize(address account, bool enabled)
        public
        onlyOwner
    {
    ...
    function isExcludedFromSellLimits(address account)
        public
        view
        returns (bool)
    {
    ...
    function setExcludedFromSellLimits(address account, bool enabled)
        public
        onlyOwner
    {
    ...
    function setWallets(address payable address1, address payable address2)
        external
        onlyOwner
    {
    ...
    function _transfer(
        ...
        if (
            lpPairs[to] &&
            balanceOf(from) > getCirculatingSupply() / 1000 &&
            !isExcludedFromSellLimits(from)
        ) {
    ...
    function contractSwap(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 cakeAmt = (contractTokenBalance * _ratios.cake) / _ratios.total;
        uint256 BNBAmt = (contractTokenBalance * _ratios.bnb) / _ratios.total;
        uint256 BUSDAmt = contractTokenBalance - (cakeAmt + BNBAmt);

        if (cakeAmt > 0) {
            _transfer(address(this), taxWallets.address1, cakeAmt);
        }
        ...
        if (BNBAmt > 0 || BUSDAmt > 0) {
            ...
            if (BNBAmt > 0) {
                taxWallets.address1.transfer(
                    ((swappedBNB * _ratios.bnb) / (_ratios.bnb + _ratios.busd))
                );
            }
            ...
            if (BUSDAmt > 0) {
                ...
                BUSD_ERC20.transfer(
                    taxWallets.address1,
                    BUSD_ERC20.balanceOf(address(this))
                );
            }
        }
    }
